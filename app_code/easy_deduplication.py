        ...
        *,
        ...
        def write_dropped(rows: List[Tuple[str, str, str, float]]):
            ...
            if dropped_writer is None:
                dropped_writer = pq.ParquetWriter(
                    f"{base}_dropped.parquet", tbl.schema, compression=self.parquet_compression
                )
        ...
        def write_clusters(rows: List[Tuple[str, str]]):
            ...
            if clusters_writer is None:
                clusters_writer = pq.ParquetWriter(
                    f"{base}_clusters.parquet", tbl.schema, compression=self.parquet_compression
                )
        ...
        # ===========================
        print(
            f"[HP] PASS A: ingest+block -> {db_path} | n_jobs_ingest={self.n_jobs_ingest if self.ingest_parallel else 1}"
        )
        global_row = 0
        prep_partial = partial(
            _prep_for_blocking, bucket_bits=self.bucket_bits, len_bin=self.len_bin, ngram=self.block_ngram
        )
        ...
        for batch in pf.iter_batches(batch_size=batch_size):
            ...
            if any(mask):
                ...
                if kept_writer is None:
                    kept_writer = pq.ParquetWriter(
                        f"{base}_kept.parquet", tbl.schema, compression=self.parquet_compression
                    )
        ...
        db.close()
        print(
            f"[HP] DONE. kept={kept_count} dropped≈{dropped_count} | files: {base}_kept.parquet, {base}_dropped.parquet, {base}_clusters.parquet, {base}_hp.sqlite"
        )
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    p.add_argument("--corpus", default="corpus.parquet", help="Wejściowy Parquet z korpusem do deduplikacji.")
    p.add_argument(
        "--relevant", default="relevant.parquet", help="Wejściowy Parquet z kolumną 'document_id' do przefiltrowania."
    )
    p.add_argument(
        "--relevant-out", default="relevant_hp_kept.parquet", help="Wyjściowy Parquet z przefiltrowanymi rekordami."
    )
    p.add_argument("--queries", default="queries.parquet", help="Wejściowy Parquet z zapytaniami (kolumna 'id').")
    p.add_argument(
        "--queries-out", default="queries_hp_kept.parquet", help="Wyjściowy Parquet z przefiltrowanymi zapytaniami."
    )
    p.add_argument(
        "--corpus-kept",
        default="corpus_hp_kept.parquet",
