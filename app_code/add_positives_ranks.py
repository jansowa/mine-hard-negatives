...
def _config_first(names: tuple[str, ...], *, cast=None, default=_MISSING):
    ...
    if default is _MISSING:
        raise UndefinedValueError(f"{', '.join(names)} not found. Declare one of these env vars or pass the CLI flag.")
    return default
...
def get_positive_ranks_stage_defaults(final_step: bool) -> dict:
    if final_step:
        return {
            "output_path": _config_first(("FINAL_POSITIVE_RANKS_OUTPUT_PATH", "RELEVANT_WITH_SCORE_PATH")),
...
def _filter_relevant_to_missing(
    ...
    if window_size:
        print(
            f"Selected window (skip={skip}, offset={offset}): {window_size:,} rows "
    ...
    try:
        ...
        with tqdm(total=rows_to_score, unit="row", desc="Scoring & writing (filtered)") as pbar:
            for batch in pf_filtered.iter_batches(batch_size=chunk_size, columns=["query_id", "document_id"]):
                ...
                pq.write_table(result_table_chunk, temp_path, compression="zstd", use_dictionary=True)
        ...
        _finalize_single_file(
            parts_dir=parts_dir, output_path=output_path, schema=relevant_extended_schema, compression="zstd"
        )
        ...
        try:
            if (
                ...
                and os.path.exists(filtered_relevant_path)
            ):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Scoring relevant pairs and saving to Parquet (crash-safe, with finalization)."
    )
    parser.add_argument(
        "--queries_path", type=str, default=config("QUERIES_PATH"), help="Path to parquet file with queries"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Path to parquet file with relevant connections",
    )
    parser.add_argument(
        "--final-step",
        "--second-step",
    ...
    parser.add_argument(
        "--chunk_size", type=int, default=config("PROCESSING_CHUNK_SIZE", cast=int), help="Chunk size for processing"
    )
    parser.add_argument(
        "--reranker_batch_size",
        type=int,
